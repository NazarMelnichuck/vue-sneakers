export const productsModule = {
	state: () => ({
		DB: [
			{
				id: 1,
				title: 'Чоловічі кросівки Nike Blazer Mid Suede',
				img: 'nike_blazer_mid_suede.jpg',
				price: 6000,
				inFavorite: false,
				inCart: false,
				size: [
					{ sizeId: 1, count: 15 },
					{ sizeId: 2, count: 0 },
					{ sizeId: 3, count: 7 },
					{ sizeId: 4, count: 34 },
					{ sizeId: 5, count: 2 },
					{ sizeId: 6, count: 48 },
					{ sizeId: 7, count: 13 },
					{ sizeId: 8, count: 28 },
					{ sizeId: 9, count: 49 },
					{ sizeId: 10, count: 19 },
				],
			},
			{
				id: 2,
				title: 'Чоловічі кросівки Nike Air Max 270',
				img: 'nike_air_max_270.jpg',
				price: 6000,
				inFavorite: false,
				inCart: false,
				size: [
					{ sizeId: 1, count: 23 },
					{ sizeId: 2, count: 5 },
					{ sizeId: 3, count: 50 },
					{ sizeId: 4, count: 12 },
					{ sizeId: 5, count: 7 },
					{ sizeId: 6, count: 40 },
					{ sizeId: 7, count: 48 },
					{ sizeId: 8, count: 8 },
					{ sizeId: 9, count: 26 },
					{ sizeId: 10, count: 41 },
				],
			},
			{
				id: 3,
				title: 'Чоловічі кросівки Nike Blazer Mid Suede',
				img: 'nike_blazer_mid_suede_white.jpg',
				price: 3500,
				inFavorite: false,
				inCart: false,
				size: [
					{ sizeId: 1, count: 32 },
					{ sizeId: 2, count: 10 },
					{ sizeId: 3, count: 5 },
					{ sizeId: 4, count: 27 },
					{ sizeId: 5, count: 8 },
					{ sizeId: 6, count: 14 },
					{ sizeId: 7, count: 46 },
					{ sizeId: 8, count: 18 },
					{ sizeId: 9, count: 37 },
					{ sizeId: 10, count: 3 },
				],
			},
			{
				id: 4,
				title: 'Кросівки Puma X Aka Boku Future Rider',
				img: 'puma_x_aka_boku_future_rider.jpg',
				price: 4000,
				inFavorite: false,
				inCart: false,
				size: [
					{ sizeId: 1, count: 6 },
					{ sizeId: 2, count: 33 },
					{ sizeId: 3, count: 14 },
					{ sizeId: 4, count: 19 },
					{ sizeId: 5, count: 42 },
					{ sizeId: 6, count: 25 },
					{ sizeId: 7, count: 11 },
					{ sizeId: 8, count: 37 },
					{ sizeId: 9, count: 2 },
					{ sizeId: 10, count: 9 },
				],
			},
			{
				id: 5,
				title: 'Чоловічі кросівки Under Armour Curry 8',
				img: 'under_armour_curry_8.jpg',
				price: 7200,
				inFavorite: false,
				inCart: false,
				size: [
					{ sizeId: 1, count: 8 },
					{ sizeId: 2, count: 43 },
					{ sizeId: 3, count: 16 },
					{ sizeId: 4, count: 38 },
					{ sizeId: 5, count: 9 },
					{ sizeId: 6, count: 4 },
					{ sizeId: 7, count: 23 },
					{ sizeId: 8, count: 30 },
					{ sizeId: 9, count: 12 },
					{ sizeId: 10, count: 47 },
				],
			},
			{
				id: 6,
				title: 'Чоловічі кросівки Nike Kyrie 7',
				img: 'nike_kyrie_7.jpg',
				price: 5000,
				inFavorite: false,
				inCart: false,
				size: [
					{ sizeId: 1, count: 49 },
					{ sizeId: 2, count: 15 },
					{ sizeId: 3, count: 41 },
					{ sizeId: 4, count: 5 },
					{ sizeId: 5, count: 32 },
					{ sizeId: 6, count: 20 },
					{ sizeId: 7, count: 17 },
					{ sizeId: 8, count: 29 },
					{ sizeId: 9, count: 7 },
					{ sizeId: 10, count: 48 },
				],
			},
			{
				id: 7,
				title: 'Чоловічі кросівки Jordan Air Jordan 11',
				img: 'nike_air_jordan_11.jpg',
				price: 6000,
				inFavorite: false,
				inCart: false,
				size: [
					{ sizeId: 1, count: 2 },
					{ sizeId: 2, count: 16 },
					{ sizeId: 3, count: 39 },
					{ sizeId: 4, count: 45 },
					{ sizeId: 5, count: 17 },
					{ sizeId: 6, count: 26 },
					{ sizeId: 7, count: 36 },
					{ sizeId: 8, count: 10 },
					{ sizeId: 9, count: 50 },
					{ sizeId: 10, count: 22 },
				],
			},
			{
				id: 8,
				title: 'Чоловічі кросівки Nike LeBron XVIII',
				img: 'nike_leborn_8.jpg',
				price: 8000,
				inFavorite: false,
				inCart: false,
				size: [
					{ sizeId: 1, count: 14 },
					{ sizeId: 2, count: 44 },
					{ sizeId: 3, count: 3 },
					{ sizeId: 4, count: 46 },
					{ sizeId: 5, count: 30 },
					{ sizeId: 6, count: 21 },
					{ sizeId: 7, count: 5 },
					{ sizeId: 8, count: 25 },
					{ sizeId: 9, count: 35 },
					{ sizeId: 10, count: 16 },
				],
			},
			{
				id: 9,
				title: 'Чоловічі кросівки Nike Lebron XVIII Low',
				img: 'nike_leborn_8_low.jpg',
				price: 6500,
				inFavorite: false,
				inCart: false,
				size: [
					{ sizeId: 1, count: 20 },
					{ sizeId: 2, count: 36 },
					{ sizeId: 3, count: 47 },
					{ sizeId: 4, count: 1 },
					{ sizeId: 5, count: 14 },
					{ sizeId: 6, count: 33 },
					{ sizeId: 7, count: 9 },
					{ sizeId: 8, count: 42 },
					{ sizeId: 9, count: 24 },
					{ sizeId: 10, count: 40 },
				],
			},
			{
				id: 10,
				title: 'Чоловічі кросівки Nike Blazer Mid Suede',
				img: 'nike_blazer_mid_suede_v2.jpg',
				price: 4000,
				inFavorite: false,
				inCart: false,
				size: [
					{ sizeId: 1, count: 28 },
					{ sizeId: 2, count: 7 },
					{ sizeId: 3, count: 19 },
					{ sizeId: 4, count: 13 },
					{ sizeId: 5, count: 38 },
					{ sizeId: 6, count: 11 },
					{ sizeId: 7, count: 50 },
					{ sizeId: 8, count: 2 },
					{ sizeId: 9, count: 22 },
					{ sizeId: 10, count: 16 },
				],
			},
			{
				id: 11,
				title: 'Чоловічі кросівки Puma X Aka Boku Future Rider',
				img: 'puma_x_aka_boku_future_rider_v2.jpg',
				price: 4200,
				inFavorite: false,
				inCart: false,
				size: [
					{ sizeId: 1, count: 24 },
					{ sizeId: 2, count: 40 },
					{ sizeId: 3, count: 31 },
					{ sizeId: 4, count: 5 },
					{ sizeId: 5, count: 14 },
					{ sizeId: 6, count: 46 },
					{ sizeId: 7, count: 8 },
					{ sizeId: 8, count: 39 },
					{ sizeId: 9, count: 16 },
					{ sizeId: 10, count: 50 },
				],
			},
			{
				id: 12,
				title: 'Чоловічі кросівки Nike Kyrie Flytrap IV',
				img: 'nike_kyrie_flytrap_4.jpg',
				price: 5000,
				inFavorite: false,
				inCart: false,
				size: [
					{ sizeId: 1, count: 4 },
					{ sizeId: 2, count: 15 },
					{ sizeId: 3, count: 34 },
					{ sizeId: 4, count: 20 },
					{ sizeId: 5, count: 9 },
					{ sizeId: 6, count: 47 },
					{ sizeId: 7, count: 26 },
					{ sizeId: 8, count: 2 },
					{ sizeId: 9, count: 40 },
					{ sizeId: 10, count: 13 },
				],
			},
		],
		sneakers: [
			{
				id: 1,
				title: 'Чоловічі кросівки Nike Blazer Mid Suede',
				img: 'nike_blazer_mid_suede.jpg',
				price: 6000,
				inFavorite: false,
				inCart: false,
				size: [
					{ sizeId: 1, count: 15 },
					{ sizeId: 2, count: 0 },
					{ sizeId: 3, count: 7 },
					{ sizeId: 4, count: 34 },
					{ sizeId: 5, count: 2 },
					{ sizeId: 6, count: 48 },
					{ sizeId: 7, count: 13 },
					{ sizeId: 8, count: 28 },
					{ sizeId: 9, count: 49 },
					{ sizeId: 10, count: 19 },
				],
			},
			{
				id: 2,
				title: 'Чоловічі кросівки Nike Air Max 270',
				img: 'nike_air_max_270.jpg',
				price: 6000,
				inFavorite: false,
				inCart: false,
				size: [
					{ sizeId: 1, count: 23 },
					{ sizeId: 2, count: 5 },
					{ sizeId: 3, count: 50 },
					{ sizeId: 4, count: 12 },
					{ sizeId: 5, count: 7 },
					{ sizeId: 6, count: 40 },
					{ sizeId: 7, count: 48 },
					{ sizeId: 8, count: 8 },
					{ sizeId: 9, count: 26 },
					{ sizeId: 10, count: 41 },
				],
			},
			{
				id: 3,
				title: 'Чоловічі кросівки Nike Blazer Mid Suede',
				img: 'nike_blazer_mid_suede_white.jpg',
				price: 3500,
				inFavorite: false,
				inCart: false,
				size: [
					{ sizeId: 1, count: 32 },
					{ sizeId: 2, count: 10 },
					{ sizeId: 3, count: 5 },
					{ sizeId: 4, count: 27 },
					{ sizeId: 5, count: 8 },
					{ sizeId: 6, count: 14 },
					{ sizeId: 7, count: 46 },
					{ sizeId: 8, count: 18 },
					{ sizeId: 9, count: 37 },
					{ sizeId: 10, count: 3 },
				],
			},
			{
				id: 4,
				title: 'Кросівки Puma X Aka Boku Future Rider',
				img: 'puma_x_aka_boku_future_rider.jpg',
				price: 4000,
				inFavorite: false,
				inCart: false,
				size: [
					{ sizeId: 1, count: 6 },
					{ sizeId: 2, count: 33 },
					{ sizeId: 3, count: 14 },
					{ sizeId: 4, count: 19 },
					{ sizeId: 5, count: 42 },
					{ sizeId: 6, count: 25 },
					{ sizeId: 7, count: 11 },
					{ sizeId: 8, count: 37 },
					{ sizeId: 9, count: 2 },
					{ sizeId: 10, count: 9 },
				],
			},
			{
				id: 5,
				title: 'Чоловічі кросівки Under Armour Curry 8',
				img: 'under_armour_curry_8.jpg',
				price: 7200,
				inFavorite: false,
				inCart: false,
				size: [
					{ sizeId: 1, count: 8 },
					{ sizeId: 2, count: 43 },
					{ sizeId: 3, count: 16 },
					{ sizeId: 4, count: 38 },
					{ sizeId: 5, count: 9 },
					{ sizeId: 6, count: 4 },
					{ sizeId: 7, count: 23 },
					{ sizeId: 8, count: 30 },
					{ sizeId: 9, count: 12 },
					{ sizeId: 10, count: 47 },
				],
			},
			{
				id: 6,
				title: 'Чоловічі кросівки Nike Kyrie 7',
				img: 'nike_kyrie_7.jpg',
				price: 5000,
				inFavorite: false,
				inCart: false,
				size: [
					{ sizeId: 1, count: 49 },
					{ sizeId: 2, count: 15 },
					{ sizeId: 3, count: 41 },
					{ sizeId: 4, count: 5 },
					{ sizeId: 5, count: 32 },
					{ sizeId: 6, count: 20 },
					{ sizeId: 7, count: 17 },
					{ sizeId: 8, count: 29 },
					{ sizeId: 9, count: 7 },
					{ sizeId: 10, count: 48 },
				],
			},
			{
				id: 7,
				title: 'Чоловічі кросівки Jordan Air Jordan 11',
				img: 'nike_air_jordan_11.jpg',
				price: 6000,
				inFavorite: false,
				inCart: false,
				size: [
					{ sizeId: 1, count: 2 },
					{ sizeId: 2, count: 16 },
					{ sizeId: 3, count: 39 },
					{ sizeId: 4, count: 45 },
					{ sizeId: 5, count: 17 },
					{ sizeId: 6, count: 26 },
					{ sizeId: 7, count: 36 },
					{ sizeId: 8, count: 10 },
					{ sizeId: 9, count: 50 },
					{ sizeId: 10, count: 22 },
				],
			},
			{
				id: 8,
				title: 'Чоловічі кросівки Nike LeBron XVIII',
				img: 'nike_leborn_8.jpg',
				price: 8000,
				inFavorite: false,
				inCart: false,
				size: [
					{ sizeId: 1, count: 14 },
					{ sizeId: 2, count: 44 },
					{ sizeId: 3, count: 3 },
					{ sizeId: 4, count: 46 },
					{ sizeId: 5, count: 30 },
					{ sizeId: 6, count: 21 },
					{ sizeId: 7, count: 5 },
					{ sizeId: 8, count: 25 },
					{ sizeId: 9, count: 35 },
					{ sizeId: 10, count: 16 },
				],
			},
			{
				id: 9,
				title: 'Чоловічі кросівки Nike Lebron XVIII Low',
				img: 'nike_leborn_8_low.jpg',
				price: 6500,
				inFavorite: false,
				inCart: false,
				size: [
					{ sizeId: 1, count: 20 },
					{ sizeId: 2, count: 36 },
					{ sizeId: 3, count: 47 },
					{ sizeId: 4, count: 1 },
					{ sizeId: 5, count: 14 },
					{ sizeId: 6, count: 33 },
					{ sizeId: 7, count: 9 },
					{ sizeId: 8, count: 42 },
					{ sizeId: 9, count: 24 },
					{ sizeId: 10, count: 40 },
				],
			},
			{
				id: 10,
				title: 'Чоловічі кросівки Nike Blazer Mid Suede',
				img: 'nike_blazer_mid_suede_v2.jpg',
				price: 4000,
				inFavorite: false,
				inCart: false,
				size: [
					{ sizeId: 1, count: 28 },
					{ sizeId: 2, count: 7 },
					{ sizeId: 3, count: 19 },
					{ sizeId: 4, count: 13 },
					{ sizeId: 5, count: 38 },
					{ sizeId: 6, count: 11 },
					{ sizeId: 7, count: 50 },
					{ sizeId: 8, count: 2 },
					{ sizeId: 9, count: 22 },
					{ sizeId: 10, count: 16 },
				],
			},
			{
				id: 11,
				title: 'Чоловічі кросівки Puma X Aka Boku Future Rider',
				img: 'puma_x_aka_boku_future_rider_v2.jpg',
				price: 4200,
				inFavorite: false,
				inCart: false,
				size: [
					{ sizeId: 1, count: 24 },
					{ sizeId: 2, count: 40 },
					{ sizeId: 3, count: 31 },
					{ sizeId: 4, count: 5 },
					{ sizeId: 5, count: 14 },
					{ sizeId: 6, count: 46 },
					{ sizeId: 7, count: 8 },
					{ sizeId: 8, count: 39 },
					{ sizeId: 9, count: 16 },
					{ sizeId: 10, count: 50 },
				],
			},
			{
				id: 12,
				title: 'Чоловічі кросівки Nike Kyrie Flytrap IV',
				img: 'nike_kyrie_flytrap_4.jpg',
				price: 5000,
				inFavorite: false,
				inCart: false,
				size: [
					{ sizeId: 1, count: 4 },
					{ sizeId: 2, count: 15 },
					{ sizeId: 3, count: 34 },
					{ sizeId: 4, count: 20 },
					{ sizeId: 5, count: 9 },
					{ sizeId: 6, count: 47 },
					{ sizeId: 7, count: 26 },
					{ sizeId: 8, count: 2 },
					{ sizeId: 9, count: 40 },
					{ sizeId: 10, count: 13 },
				],
			},
		],
		filterItems: [
			{ text: 'Популярні', value: 'popular' },
			{ text: 'Від дешевих до дорогих', value: 'toRich' },
			{ text: 'Від дорогих до дешевих', value: 'toCheap' },
		],
		productPage: {},
		productSizeValues: {
			ua: [
				{
					sizeId: 1,
					value: 40,
				},
				{
					sizeId: 2,
					value: 41,
				},
				{
					sizeId: 3,
					value: 41.5,
				},
				{
					sizeId: 4,
					value: 42,
				},
				{
					sizeId: 5,
					value: 43,
				},
				{
					sizeId: 6,
					value: 43.5,
				},
				{
					sizeId: 7,
					value: 44,
				},
				{
					sizeId: 8,
					value: 44.5,
				},
				{
					sizeId: 9,
					value: 45,
				},
				{
					sizeId: 10,
					value: 46,
				},
			],
			us: [
				{
					sizeId: 1,
					value: 8,
				},
				{
					sizeId: 2,
					value: 8.5,
				},
				{
					sizeId: 3,
					value: 9,
				},
				{
					sizeId: 4,
					value: 9.5,
				},
				{
					sizeId: 5,
					value: 10,
				},
				{
					sizeId: 6,
					value: 10.5,
				},
				{
					sizeId: 7,
					value: 11,
				},
				{
					sizeId: 8,
					value: 11.5,
				},
				{
					sizeId: 9,
					value: 12,
				},
				{
					sizeId: 10,
					value: 12.5,
				},
			],
			eu: [
				{
					sizeId: 1,
					value: 41,
				},
				{
					sizeId: 2,
					value: 42,
				},
				{
					sizeId: 3,
					value: 42.5,
				},
				{
					sizeId: 4,
					value: 43,
				},
				{
					sizeId: 5,
					value: 44,
				},
				{
					sizeId: 6,
					value: 44.5,
				},
				{
					sizeId: 7,
					value: 45,
				},
				{
					sizeId: 8,
					value: 45.5,
				},
				{
					sizeId: 9,
					value: 46,
				},
				{
					sizeId: 10,
					value: 47,
				},
			],
			uk: [
				{
					sizeId: 1,
					value: 7,
				},
				{
					sizeId: 2,
					value: 7.5,
				},
				{
					sizeId: 3,
					value: 8,
				},
				{
					sizeId: 4,
					value: 8.5,
				},
				{
					sizeId: 5,
					value: 9,
				},
				{
					sizeId: 6,
					value: 9.5,
				},
				{
					sizeId: 7,
					value: 10,
				},
				{
					sizeId: 8,
					value: 10.5,
				},
				{
					sizeId: 9,
					value: 11,
				},
				{
					sizeId: 10,
					value: 11.5,
				},
			],
			cm: [
				{
					sizeId: 1,
					value: 26,
				},
				{
					sizeId: 2,
					value: 26.5,
				},
				{
					sizeId: 3,
					value: 27,
				},
				{
					sizeId: 4,
					value: 27.5,
				},
				{
					sizeId: 5,
					value: 28,
				},
				{
					sizeId: 6,
					value: 28.5,
				},
				{
					sizeId: 7,
					value: 29,
				},
				{
					sizeId: 8,
					value: 29.5,
				},
				{
					sizeId: 9,
					value: 30,
				},
				{
					sizeId: 10,
					value: 30.5,
				},
			],
		},
	}),

	getters: {
		getCountrySize: (state) => {
			return Object.keys(state.productPage.size)
		},
		getProductSize: (state) => (sizeCountry) => {
			return Object.values(state.productPage.size[sizeCountry])
		},
		getProductCount: (state) => (productId, sizeId) => {
			const product = state.sneakers.find((product) => product.id === productId)

			if (product) {
				const size = product.size.find((size) => size.sizeId === sizeId)

				if (size) {
					return size.count
				}
			}
		},
		getProductCartStatus: (state) => (productId) => {
			const product = state.sneakers.find((product) => product.id === productId)

			if (product) {
				return product.inCart
			}
		},
	},

	mutations: {
		setSneakers(state, newSneaker) {
			state.sneakers = newSneaker
		},
		// addToFavorites(state, product) {
		// 	product.inFavorite = true
		// },
		// deleteFromFavorites(state, product) {
		// 	product.inFavorite = false
		// },
		setFavoritesStatus(state, { id, inFavorite, inProductPage = false }) {
			state.sneakers.find((el) => {
				if (el.id === id) {
					el.inFavorite = inFavorite

					if (inProductPage) {
						this.commit('getProduct', id)
					}

					if (inFavorite) {
						this.commit('addToFavorites', el)
					} else {
						this.commit('deleteFromFavorites', el)
					}
				}
			})
		},
		addCart(state, productId) {
			state.sneakers.find((el) => {
				if (el.id === productId) {
					el.inCart = true
				}
			})
		},
		deleteCart(state, productId) {
			state.sneakers.find((el) => {
				if (el.id === productId) {
					el.inCart = false
				}
			})
		},
		minusCountProductSize(state, { id, sizeId }) {
			state.sneakers.forEach((product) => {
				if (product.id === id) {
					product.size.forEach((size) => {
						if (size.sizeId === sizeId) {
							size.count -= 1
						}
					})
				}
			})
		},
		getProduct(state, id) {
			state.productPage = { ...state.sneakers.find((product) => product.id === id) }

			const newSize = {}

			for (let countryKey in state.productSizeValues) {
				newSize[countryKey] = state.productSizeValues[countryKey].map((sizeValue) => {
					const correspondingSize = state.productPage.size.find((productSize) => {
						return productSize.sizeId === sizeValue.sizeId
					})
					return {
						sizeId: sizeValue.sizeId,
						value: sizeValue.value,
						count: correspondingSize ? correspondingSize.count : 0,
					}
				})
			}
			state.productPage.size = newSize
		},
		setSizeCountry() {},
		searchProduct(state, searchValue) {
			if (searchValue.length === 0) {
				state.sneakers = [...state.DB]
			} else {
				state.sneakers = state.DB.filter((el) =>
					el.title.toLowerCase().includes(searchValue.toLowerCase())
				)
			}
		},
		filterProducts(state, filterType) {
			switch (filterType) {
				case 'popular':
					state.sneakers = [...state.DB]
					break
				case 'toCheap':
					state.sneakers.sort(function (a, b) {
						return b.price - a.price
					})
					break
				case 'toRich':
					state.sneakers.sort(function (a, b) {
						return a.price - b.price
					})
					break
				default:
					break
			}
		},
		deleteCartAfterOrder(state, cartItems) {
			cartItems.forEach((cartItem) => {
				state.sneakers.forEach((storeItem) => {
					if (storeItem.id === cartItem.id) {
						storeItem.inCart = false
					}
				})
			})
		},
	},
}
